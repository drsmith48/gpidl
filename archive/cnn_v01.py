...
import sys, psutil, pickle, time, random
...
def trainnet(data=None,
    ...
    labels = obj['frameinfo']['category'].to_numpy()
    ...
    train_set = TensorDataset(torch.from_numpy(frames[itrain,...]),
                              torch.from_numpy(labels[itrain]))
    train_loader = DataLoader(train_set, batch_size=batch_size)
    test_frames = torch.from_numpy(frames[itest,...]).to(device)
    test_labels = torch.from_numpy(labels[itest]).to(device)
    validate_frames = torch.from_numpy(frames[ivalidate,...]).to(device)
    validate_labels = torch.from_numpy(labels[ivalidate]).to(device)
    
    # reset (randomize) model parameters
    for mod in enumerate(net.modules()):
        if hasattr(mod, 'reset_parameters'):
            mod.reset_parameters()

    print('Begin training with {} epochs'.format(epochs))
    ...
    for epoch in range(epochs):
            ...
            if i%superbatch == 0:
                with torch.no_grad():
                    ...
                    print('Epoch {}/{}  batch {:06d}  test loss {:.6e}'.
                          format(epoch+1,epochs,i,test_loss.item()))
                    test_losses = np.append(test_losses, test_loss.item())
                    if test_losses.size>32:
                        curr_loss = np.median(test_losses[-5:])
                        past_loss = np.median(test_losses[-30:-25])
        ...
        if epoch_break:
            break
    final_epoch = epoch+1
    print('End training in epoch', final_epoch)
    ...
    with torch.no_grad():
        ...
        final_accuracy = ncorrect/ntotal*100
        print('Accuracy: {}/{} ({:.2f}%)'.format(ncorrect, ntotal, final_accuracy))

    return final_accuracy

        
def gpu_worker(itask, iworker, igpu, train_kwargs, result_queue):
    ...
    full_result = (itask, iworker, igpu, delta_seconds, result, train_kwargs)
...
def batch_training():
    ...
    for i,result in enumerate(results):
        print('Task {:4d} worker {:2d} GPU {:2d} dt {:5.1f} s  acc {:5.2f}%  kw: {}'.
            format(*result))
    delta_seconds = time.time() - createtime
