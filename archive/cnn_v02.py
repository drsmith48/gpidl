...
def trainnet(data=None,
    ...
    with torch.no_grad():
        ...
        print('Accuracy: {}/{} ({:.2f}%)'.format(ncorrect, ntotal, accuracy))
...
def gpu_worker(itask, iworker, igpu, train_kwargs, result_queue):
    ...
    full_result = (itask, iworker, igpu, result['accuracy'], delta_seconds, 
...
def batch_training():
    ...
    for i,result in enumerate(results):
        print('Task {:4d} worker {:2d} GPU {:2d} acc {:5.2f}% dt {:5.1f}s epochs {:2d} kw: {}'.
            format(*result))
    delta_seconds = time.time() - createtime
